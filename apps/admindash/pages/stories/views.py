...
class StoriesCreateView(CreateView):
    ...
    success_url = reverse_lazy('admindash:pages:stories:list')

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['active'] = 'master'
        context['current'] = 'stories'
        return context

    def form_valid(self, form):
        default_category = Category.objects.get(name='stories')
        form.instance.category = default_category
        messages.success(self.request, 'stories created successfully.')
...
class StoriesListView(ListView):
    ...
    def get_queryset(self):
        ...
        queryset = queryset.filter(category__name="stories")
        return queryset
...
class StoriesUpdateView(UpdateView):
    model = Title
    form_class = StoriesForm
    template_name = 'stories/update.html'
    success_url = reverse_lazy('admindash:pages:stories:list')
    ...
    def form_valid(self, form):
        messages.success(self.request, 'stories updated successfully.')
        return super().form_valid(form)
...
class StoriesDeleteView(DeleteView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        # Delete the object
        obj.delete()
        messages.success(self.request, 'Stories deleted successfully.')
